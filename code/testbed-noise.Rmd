---
title: 'Testbed - ten noise variables'
...
set.seed(77)
ggplot(data = filter(nested_df, ID %in% sample(1:1000, 25))) +
...
Q_glmnet <- Qlearn(
  data = dat_long,
  formula = Q_hat ~ tumor_mass + dose + toxicity + V1 + dose * tumor_mass + dose * toxicity + dose * V1,
  treatment = "dose",
  mod_type = "caret",
  na.action = na.omit,
  method = "glmnet"
...
Q_ert <- Qlearn(
  data = dat_long,
  formula = Q_hat ~ tumor_mass + dose + toxicity + V1 + V2 + V3 + V4 + V5 + V6 + V7 + V8 + V9 + V10,
  treatment = "dose",
  mod_type = "caret",
  method = 'extraTrees',
  na.action = na.omit,
