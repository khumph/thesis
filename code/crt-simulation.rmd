...
library(pacman)
p_load(tidyverse, rms, stringr, gridExtra, rpart)
```
...
tab <- dat_test_long %>% group_by(group) %>% summarise(num_died = sum(died, na.rm = T), prop_died = num_died/200)
list(plot_tox = plot_tox, plot_tumor = plot_tumor, plot_sum = plot_sum, table_deaths = tab)
}
```
...
dat_test_long <- sim_test(Q)
```

```{r}
dat_test_long %>% plots_tab()
```
