...
# # function for how tumor mass changes
# Mdot <- function(M, W, D, X, a2 = 0.15, b2 = 1.2, d2 = 0.5) {
#   (a2 * W - b2 * (D - d2)) * ifelse(M > 0, 1, 0) + ifelse(X > 0.5 & D > 0.75, -2, 0)
# }

# # function for how tumor mass changes
# Mdot <- function(M, W, D, X, a2 = 0.15, b2 = 1.2, d2 = 0.5) {
#   (a2 * W - b2 * (D * ifelse(X > 0.5 & D > 0.5, 2, 0) - d2)) * ifelse(M > 0, 1, 0)
# }
...
# reward functions
R2 <- function(W1, W0, reward.type = "orig") {
  if (reward.type == "orig") {
    ifelse(W1 - W0 <= -0.5, 5,
...
R3 <- function(M1, M0, reward.type = "orig") {
  if (reward.type == "orig") {
    ifelse(M1 == 0 & M0 == 0, 0,
           ifelse(M1 == 0, 15,
                  ifelse(
...
sim <- function(N = 1000, Ttot = 6, reward.type) {
  # 1000 patients
  # N <- 1000
  # 6 months treatment
  # Ttot <- 6 
