...
### Initialize $Q(s, a)$, for all $s \in \mathcal{S}$, $a \in \mathal{A}(s)$, arbitrarily, and $Q(\texttt{terminal_state}, \cdot) = 0$ 
```{r}
dat_long <- dat_long %>% mutate(Q = 0)
```
...
2. $Q(S, A) \leftarrow Q(S, A) + \alpha * (R + \gamma * \max_a Q(S',a) - Q(S, A))$

```{r}
df <- function(occasion, data = dat_long) {
  filter(data, month == occasion)
}

fit_rcs <- function(occasion) {
  ols(
    reward ~
      rcs(tumor_mass) +
      rcs(dose) +
...
get_preds <- function(x, df) {
  df <- df %>% select(tumor_mass, toxicity) %>% mutate(dose = rep(x, 1000))
  predict(fit_rcs(5), df)
}
...
ggplot(df(5), aes(x = predict(mod), y = residuals(mod, "dffits"))) + geom_point()

ggplot(df(5), aes(x = tumor_mass, y = reward)) + geom_point() +
  geom_point(aes(x = tumor_mass, y = predict(mod)), color = "blue")

ggplot(df(5), aes(x = toxicity, y = reward)) + geom_point() +
  geom_point(aes(x = toxicity, y = predict(mod)), color = "blue")

ggplot(df(5), aes(x = predict(mod), y = reward)) + geom_point()
```
...
get_preds <- function(x, df) {
  df <- df %>% select(tumor_mass, toxicity) %>% cbind(dose = rep(x, 1000))
  predict(mod, df)
}
...
t5 <- df(5) %>% select(ID, tumor_mass, toxicity) %>% bind_cols(preds) %>%
  gather(dose, pred, one_of(x_char), -ID) %>% 
  mutate(dose = as.numeric(dose)) %>% 
  group_by(ID, tumor_mass, toxicity) %>% nest() %>% 
  bind_cols(
