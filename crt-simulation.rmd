...
The treatment set consists of doses of a chemotherapy agent with an acceptable dose range of [0, 1], where the value 1 corresponds to the maximum acceptable dose.  
...
nest_max_df <- function(df, preds) {
  df %>%
    ...
    mutate(
      ...
      dose_optim = ifelse(is.na(max), NA, map_dbl(.$data, ~ min((which.max(ifelse(is.na(.$pred), 999, .$pred)) - 1) / 100)))
    )
}
