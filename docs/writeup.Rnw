...
dat_summ %>%
  filter(.data$scenario %in% grep('Interaction', labs_lng, value = T, invert = T)) %>%
  plot_surv()
@
...
dat_summ %>%
  filter(.data$scenario %in% grep('Interaction', labs_lng, value = T)) %>%
  plot_surv()
@

<<tab-surv, results='asis'>>=
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{extras/efigure/var-imps-no-int-1}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{extras/efigure/var-imps-int-1}
