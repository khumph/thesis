...
This produces the tree in Figure \ref{fig:ex-cart-tree}. Since the regression function is actually linear, the splits are intuitive: first we split the root node (node 1) at around the midpoint of the distribution of doses ($D = \Sexpr{round(mod$finalModel$splits[1, 'index'], 3)}$), then we split the observations with doses higher than \Sexpr{round(mod$finalModel$splits[1, 'index'], 3)} (node 2) at about their median dose, \Sexpr{round(mod$finalModel$splits[2, 'index'], 3)}, producing the terminal nodes 3 and 4. Similarly with doses less than \Sexpr{round(mod$finalModel$splits[1, 'index'], 3)} (node 5), we split at approximately the median dose (\Sexpr{round(mod$finalModel$splits[3, 'index'], 3)}), yielding terminal nodes 6 and 7.
