\RequirePackage[l2tabu, orthodox]{nag} % 12tabu: warn for obsolete commands/packages, orthodox: check for correct commands that may give undesirable results
...
\usepackage{tabu} % more flexible table environment
\usepackage{amsmath} % math
\usepackage{commath} % lots of math mode conveniences
\usepackage[]{algorithm2e} % algorithm environment
\RestyleAlgo{boxruled} % put algorithms in a box
\setlength\belowcaptionskip{12pt} % add space before table and figure captions
...
% \begin{multline}
%     Q^{*}_{k}(s, a) = \operatorname{E}\sbr{R_{k+1} \mid S_k = s, A_k = a} + \\ \sum_{s' \in \mathcal{S}} \max_{a'} Q^{\pi}_{k+1}(s', a') % \pi(s', a')
%     P(S_{k+1} = s' \mid S_k = s, A_k = a)
% \end{multline}
% Where \( \pi(s', a') \) is the probability of taking action \(a'\) in state \(s'\) as dictated by policy \(\pi\).
...
\begin{algorithm}[ht]
  initialize \(\hat{Q}^{\pi}_{K + 1}\) arbitrarily:

  \(\hat{Q}^{\pi}_{K + 1} \gets 0\)
...
\( \hat{r}_{k} \gets r_{k} + \max_{A_{k+1}} \hat{Q}^{\pi}_{k+1}(A_{k+1}, H_{k+1})\)

\medskip

Estimate the Q-function, \(Q^{\pi}_{k}(A_{k}, H_{k})\), for the preceding stage using this reward as the outcome (e.g. through ordinary least squares).

\medskip

Estimate the optimal treatment(s) for each patient at stage \(k\):

\(\hat{A}^{*}_{k} \gets \argmax_{A_{k}} \hat{Q}_{k}(A_{k}, H_{k})\)
 }
...
<<ex-cart-tree, fig.cap=caption, fig.scap='Regression tree fit to example data', fig.width='0.9\\textwidth', fig.height=5, dev.args=list(pointsize=13)>>=
...
This produces the tree in Figure \ref{fig:ex-cart-tree}. Since the regression function is actually linear, the splits are intuitive: first we split the root node (node 1) at around the midpoint of the distribution of doses (\(D = \Sexpr{splits[1]}\)), then we split the observations with doses higher than \Sexpr{splits[1]} (node 2) at about their median dose, \Sexpr{splits[2]}, producing the terminal nodes 3 and 4. Similarly with doses less than \Sexpr{splits[1]} (node 5), we split at approximately the median dose (\Sexpr{splits[3]}), yielding terminal nodes 6 and 7.

<<ex-plot-cart, fig.cap=caption, fig.scap='Predicted regression function for example data using CART', out.width='0.75\\textwidth', fig.height=3, fig.width=5>>=
...
<<ex-plot-mars, fig.cap=cap, fig.scap='Predicted regression function for example data using MARS', out.width='0.75\\textwidth', fig.height=3, fig.width=5>>=
...
\begin{equation}
  \label{eq:mars-eqn}
  \hat{\operatorname{E}}[Y \mid D] \approx \Sexpr{coefs[1]} + \Sexpr{coefs[2]} (\Sexpr{cuts[2]} - D)_{+} \Sexpr{coefs[3]} (D - \Sexpr{cuts[3]})_{+}
\end{equation}
Note that the hinges don't meet because the hinges that entered paired with each remaining hinge were removed in the pruning/backward pass phase. The gap between hinges is shown in our plot as a horizontal line of \(Y = \Sexpr{coefs[1]}\) for \(\Sexpr{cuts[2]} < D < \Sexpr{cuts[3]}\).

Using this model to predict the testing data yields a root mean squared error of \Sexpr{round(pR["RMSE"], 3)} and a coefficient of determination (\(R^2\)) of \Sexpr{round(pR["Rsquared"], 3)}. As these measures indicate, MARS does a much better job of capturing the linear nature of the relationship between \(Y\) and \(D\) than CART does, with many fewer ``splits''. Note also that we can restrict MARS to only consider predictors to enter as linear terms (without splitting into hinge functions), which we would probably do here given that the fitted regression line is nearly linear anyway, and would then be easier to interpret.
...
<<ex-plot-rf, fig.cap=cap, fig.scap='Predicted regression function for example data using Random Forest', out.width='0.75\\textwidth', fig.heigh=3, fig.width=5>>=
...
When using this model to predict the test data we obtain a root mean squared error of \Sexpr{round(pR['RMSE'], 3)} and an \(R^2\) of \Sexpr{round(pR['Rsquared'], 3)}. This compares much more favorably to MARS than a single tree, however it comes at the cost of a lack of clarity in the relationship between \(Y\) and \(D\) in the model.
...
<<results-no-int, fig.cap=caption, fig.scap='Mean months of survival under each regime for scenarios without interaction', fig.width='\\textwidth', fig.height=7>>=
...
<<results-int, fig.cap=caption, fig.scap='Mean months of survival under each regime for scenarios with interaction', fig.width='\\textwidth', fig.height=7>>=
...
<<var-imps-no-int, fig.cap = cap, fig.scap='Variable importances for the scenarios without interaction', fig.height=7, fig.width='\\textwidth', out.width='\\textwidth'>>==
...
<<var-imps-int, fig.cap = cap, fig.scap='Variable importances for interaction scenarios', fig.height=7, fig.width='\\textwidth', out.width='\\textwidth'>>==
...
Hmisc::latex(select(imp_tab, -var_grp), file = "", dec = 1, na.blank = T,
      caption = cap, caption.lot = cap_toc, label = 'tab:imp',
