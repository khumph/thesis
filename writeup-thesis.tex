...
MARS (\cite{mars}) can be viewed as a modification of the CART procedure to improve performance in the regression setting, or as a generalization of stepwise linear regression methods that incorporates automatic modelling of interactions. MARS accomplishes this latter feature through modelling covariates as piecewise linear basis functions and their products. These piecewise linear basis functions are of the form
%
...
Survival times (in months) at month $t$ were generated from an exponential distribution. The rate parameter for ths distribution was modeled as a function of the tumor mass and toxicity which would result at time $t+1$:
\begin{equation}
...
\begin{description}
  ...
  \item [CART, MARS, RF:] Out of doses from the set $\{0, 0.01, 0.02, \ldots, 1\}$, the dose corresponding to the maximum predicted rewards for each of the 20 CART, MARS, and random forest models for each stage.
\end{description}
...
For ease of presentation, the interaction variables, $X$, are be presented together as a group. Each noise variable vector, $Z$ and $V$, are also presented as groups, since in each scenario the importances of each variable within each vector should be the same. Importances for each of the $X$, $Z$, and $V$ vectors were separately averaged across the 20 models for each technique and, since the importances were very similar across stages, averaged across stages as well. Standard deviations of importances across stage, training set, and variable vector were also calculated to show the variability of importance.
...
Figures \ref{fig:var-imps-no-int} and \ref{fig:var-imps-int} and Table \ref{tab:imp} also show that this version of MARS is much better than the other modelling techniques at identifying interacting variables and the treatment ($D$) as important. However, it is also much more prone to identify noise variables as important. As with mean survival times, there is higher variance across training sets in the importances MARS assigns to treatment and interacting variables. Random forest is best at identifying the noise variables as not important, but also gives a relatively low importance to interacting and treatment variables. A single regression tree (CART) shows a similar trend in importances as the random forest models, but with more variability and higher importances on dose and lower importances on interacting variables.
