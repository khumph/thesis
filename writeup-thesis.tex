...
Patient toxicity measured at a given month $t$, $W_{t}$, which I define as the negative of wellness or quality of life\footnote{For simplicity, I take wellness and quality of life to be equivalent and exactly the opposite of toxicity}, was modeled as a function of tumor mass and dosage of treatment:
%
\begin{equation}
...
\end{equation}
%
for a given $W_{t-1}$, $D_{t-1}$ above 0.5 decreases $M_{t}$ and $D_{t-1}$ below 0.5 allows $M_{t}$ to increase, with $M_{t}$ bounded by 0 since mass is strictly positive.
...
To generate the training data, I began by simulating 1000 patients, each receiving a total of three months of treatment ($t = 0, 1, 2$), the doses of which were chosen from a uniform distribution over 0 and 1 at the beginning of each month:
\begin{equation}
...
\begin{description}
  ...
  In the pruning step, the $T_{\alpha}$ with the smallest 10 fold cross validation error was chosen as the final tree. This was achieved via the \texttt{rpart} package in \textsf{R}.
...
Initial tumor masses ($M_{0}$) and toxicities ($W_{0}$) for 2000 new patients, were simulated identically to those in each scenario for the training data. Copies of these individuals (i.e. 2000 each copy with the same initial tumor mass and toxicities) were made to undergo one of the following regimes:
...
The transition and survival functions were identical to the training data, with one exception: to increase the comparability between dosing schemes, the expected survival time was used as the actual survival time (and the determination of a participant had died), rather than the survival time using random draws from an exponential distribution.

Variable importance was calculated for each model at each stage

arithmetic mean survival times calculated, etc
