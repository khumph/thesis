...
\[
  \lambda_{t}(M_{t}, W_{t}) = \exp(-7 + W_{t} + 1.2 M_{t} + W_{t} M_{t}).
\]
...
To generate training data, I began by simulating 1000 patients, each receiving six months of treatment, the doses of which were chosen from a uniform distribution over 0 and 1 at the beginning of each month:

\[
  D_{t} \overset{iid}{\sim} \text{Unif}(0, 1)
\]

Each patient's initial tumor mass and toxicity were generated independently from uniform distributions between 0 and 2.
...
At each month, the next month's tumor mass and toxicity were computed in response to dose for each patient as described above. The rate parameter for the survival distribution was updated for each patient from these next month's tumor masses and toxicities. Survival times were then randomly sampled from the relevant exponential distribution. If this survival time was less than 1 (i.e. the patient would not survive another month) the patient was said to have died before the next month, and the reward received from this patient was computed as the log of the total number of months the patient had survived (including the fraction of a month before their death). 
