...
Since in reinforcement learning rewards are defined to be continuous, we focus on regression trees constructed using the CART procedure of \textcite{CART}. 
...
\begin{equation}
  \lambda_{t}(M_{t+1}, W_{t+1}) = \exp(-6.5 + W_{t+1} + 1.2 M_{t+1} + 0.8 W_{t+1} M_{t+1}).
\end{equation}
...
\begin{equation}
  S_{t} \sim \text{Exp}(\exp(-6.5 + W_{t+1} + 1.2 M_{t+1} + 0.8 W_{t+1} M_{t+1}))
\end{equation}
...
\begin{equation}
  \lambda'_{t}(M_{t+1}, W_{t+1}) = \exp\del{-6.5 + W_{t+1} + 1.2 M_{t+1} + 0.8 W_{t+1} M_{t+1} + 0.05 \sum_{i = 1}^{10} Z_{i}}
\end{equation}
...
Starting conditions for 200 new patients were simulated identically to patients in the training data. We made 15 copies of these individuals (i.e. each copy with the same initial tumor mass and toxicities) who would undergo one of the following regimes:
...
% Notches extend $1.58 \times IQR / \sqrt{n}$, giving a rough 95\% confidence interval for medians. 
...
%latex.default(r_tab, file = "", colheads = labs, booktabs = T)%
\begin{table}[!tbp]
...
\toprule
% \multicolumn{1}{c}{Regime}&\multicolumn{1}{c}{Basic}&\multicolumn{1}{c}{Noise}&\multicolumn{1}{c}{\begin{tabular}{c} Predictive \\ noise \end{tabular}} &\multicolumn{1}{c}{Interaction}&\multicolumn{1}{c}{\begin{tabular}{c} Int. \& \\ noise \end{tabular}}&\multicolumn{1}{c}{\begin{tabular}{c} Int. \& \\ pred. noise \end{tabular}}\tabularnewline
\multicolumn{1}{c}{Regime}&\multicolumn{1}{c}{B}&\multicolumn{1}{c}{N}&\multicolumn{1}{c}{PN}&\multicolumn{1}{c}{I}&\multicolumn{1}{c}{\begin{tabular}{c} I + N \end{tabular}}&\multicolumn{1}{c}{I + PN}\tabularnewline
\midrule
...
This makes subgroup identification difficult, relegating the endeavor to simply examining variable importance measures, which indicate the importance in terms of predictive accuracy, not necessarily that a variable identifies a subgroup with differential treatment effects.
