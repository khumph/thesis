...
\begin{algorithm}[H]
  initialize $\hat{Q}_{K + 1}^{opt}$ arbitrarily:
\[
    \hat{Q}_{K + 1}^{opt} \gets 0
\]
 \For{$k \in K, \ldots, 1$}{
...
As mentioned before, trees have many attractive features, particularly when the goal is obtaining subgroups, as will be discussed in the last part of this section. Since in reinforcement learning rewards are defined to be continuous we consider the regression tree version of the CART procedure in the discussion below.
...
We use the the classification and regression tree (CART) approach of \textcite{CART}. We begin with all of the data (the \emph{root node}) and evaluate the sum of squared errors that result from splitting each unique value of each covariate:
\begin{equation}
...
\For{each internal node}{
  ...
  compute cost-complexity criterion value
}
\For{each subtree $T$ with the same number of terminal nodes $\abs{T}$}{

}
Find nodes that, if collapsed, produce the smallest increase in SSE per node (weakest link), these are the $T_{\alpha}$s

\For{each $T_{\alpha}$}{
 calculate the cross validation error
}
Choose the largest $T_{\alpha}$ within one standard error of the minimum cross validation error
...
\end{equation}
Where $\epsilon_{i} \sim N(0, 0.05)$. 100 patients were simulated with random starting tumor masses sampled from a uniform distribution from 1 to 2, $M_{0} \overset{iid}{\sim} \text{Unif}(1, 2)$, and doses were sampled from a uniform distribution between 0 (none) and 1 (the maximum tolerable dose), $D \overset{iid}{\sim} \text{Unif}(0, 1)$.

Fitting a regression tree to the example data yields the following tree
...
\subsection{Variable importance in random forests} % (fold)
\label{sub:variable_importance_in_random_forests}

% subsection variable_importance_in_random_forests (end)

\subsection{Advantages and disadvantages to random forest} % (fold)
\label{sub:advantages_and_disadvantages_to_random_forest}

This approach leads to significantly more accurate predictions over a single tree, but also significantly reduces the interpretability. This makes subgroup identification difficult, relegating the endeavor to simply examining variable importance measures, which indicate the importance in terms of predictive accuracy, not necessarily that a variable identifies a subgroup with differential treatment effects.
...
Largely inspired by \textcite{crt}
...
\begin{description}
  ...
  \item [MARS:] Only interactions with the treatment variable, \texttt{dose}, were allowed, and these were restricted to second degree interactions. The forward pass was stopped if more than 200 terms were created, or further terms would decrease change in SSE? of less than 0.001. The number of terms in the model, $\lambda$, was chosen using generalized cross validation. 
  \item [RF:] In all scenarios, \texttt{dose} was forced to be considered at each split.
  \begin{description}
...
For validation data, I simulated the starting conditions for 200 new patients in the same way as for the training data. I made 15 copies of these individuals (i.e. each copy with the same initial tumor mass and toxicities) who would undergo one of the following regimes:
...
\begin{description}
  ...
  \item [``Best'':] the greedy dose choice given complete knowledge of the transition functions and their influence on survival
  \item [CART, MARS, RF:] The estimated best doses (ones that maximize expected survival) from each of the three methods to estimate Q-functions: CART, MARS, and random forest
\end{description}
...
\end{center}
\caption[Months of survival under the interaction scenario]{Months of survival under each regime for the interaction scenario.}
\label{fig:results-int}
\end{figure}

\begin{figure}[H]
\begin{center}
\includegraphics[width=\textwidth]{results-plot-noise.png}
\end{center}
\caption[Months of survival under the noise scenario]{Months of survival under each regime for the noise scenario.}
\label{fig:results-noise}
\end{figure}

\begin{figure}[H]
\begin{center}
\includegraphics[width=\textwidth]{results-plot-noise-pred.png}
\end{center}
\caption[Months of survival under the predictive noise scenario]{Months of survival under each regime for the predictive noise scenario.}
\label{fig:results-noise}
\end{figure}

\begin{figure}[H]
\begin{center}
\includegraphics[width=\textwidth]{results-plot-int-noise.png}
\end{center}
\caption[Months of survival under the interaction and noise scenario]{Months of survival under each regime for the interaction and noise scenario.}
\label{fig:results-int-noise}
\end{figure} 

\begin{figure}[H]
\begin{center}
\includegraphics[width=\textwidth]{results-plot-int-noise-pred.png}
\end{center}
\caption[Months of survival under the interaction and predictive noise scenario]{Months of survival under each regime for the interaction and predictive noise scenario.}
\label{fig:results-int-noise-pred}
\end{figure}
